...
**Note**: In Python the division operator `/` will actually perform a floor division if the variables being divided are both integers.  So whole numbers `10 / 3` gives `3` whereas floating point `10.0 / 3.0` gives `3.33` recurring.  You can also force floor division using the `//` operator if needed.
...
Notice the use of the `in` keyword on the Python side.  The variable `lst` is actually a list containing the integers 1 to 3.  It's created by the range function by passing in the number to start from and the number to stop at; stopping at 4, therefore, leaves 3 as the final number.
